...
namespace PacketDefinitions420
{
    ...
    public class PacketServer
    {
        ...
        public PacketServer(ushort port, string[] blowfishKeys, bool logOutPackets, bool logInPackets)
        {
            ...
            {
                byte[] key = Convert.FromBase64String(blowfishKeys[i]);
                if (key.Length <= 0)
                {
            ...
            PacketHandlerManager = new PacketHandlerManager(blowfishes, _server, logOutPackets, logInPackets);
        }

        internal int GetPort()
        {
            return (_server.Socket.LocalEndPoint as IPEndPoint)?.Port ?? 0;
        }
