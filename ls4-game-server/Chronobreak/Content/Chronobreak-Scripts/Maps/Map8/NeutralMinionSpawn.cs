...
public class NeutralMinionSpawn
{
    ...
    public static void InitializeNeutrals()
    {
        ...
        foreach (var camp in SpeedShrines.Keys)
        {
            if (!camp.IsAlive)
            {
                AddPosPerceptionBubble(camp.Position.ToVector2(), 250.0f, 1.0f, TeamId.TEAM_BLUE);
                AddPosPerceptionBubble(camp.Position.ToVector2(), 250.0f, 1.0f, TeamId.TEAM_PURPLE);
