    ...
    <div class="storecontent">
        <div class="itemlist">
            ...
            <div class="item" v-for="(item, index) in items" :key="index">
                <div class="title">
                    {{ item.name ? item.name : `Skin ${item.id}` }}
                </div>
                <div class="content">
...
export default {
    ...
    methods: {
        getStyleProp(item) {
            ...
            return `background-image: url("${host}:${port}/skins/${item.championId}/loading/${item.championId}_${item.pictureId}.jpg")`;
        },
        canBuy(item) {
        ...
        buyItem(item) {
            this.$store
                .dispatch("postPurchaseItem", { item, category: "Skins" })
                .then(response => {
                    if (response.status === 200) {
                        this.$sound.template("CORE_SHOP_BUY_CHAMPION");
                        this.$store.dispatch("getStoreItems", "skins").then(items => {
                            this.items = items;
                        });
