...
const __STATIC = window.electron.ipcRenderer.invoke("getStatic").then(result => {
    return result;
});
                ...
                methods: {
                    ...
                    printMissing() {
                        ...
                        if (Object.keys(doesntHave).length > 0) {
                            window.electron.ipcRenderer.send("translate_missing_langkeys", doesntHave);
                        }
                    },
                    ...
                    loadLanguagesInternal(langDir, langFolderPath, defaultLang) {
                        ...
                        langDir.forEach(file => {
                            const fileContents = fs.readFileSync(join(langFolderPath, file), "utf-8");
